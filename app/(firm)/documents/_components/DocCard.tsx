...
export default function DocCard({ doc: d, saving, acting, onStatusChange, onEditNote, onView, onDownload, onEvents }: { doc: DocRow } & CardHandlers) {
  const isLivr = d.type && !d.type.customer
  return (
    <div style={{ background: '#fff', border: `1px solid ${isLivr ? '#BFDBFE' : '#E2E8F0'}`, borderRadius: '8px', padding: '12px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '2px' }}>
            <span style={{ fontSize: '11px', fontWeight: 600, color: '#64748B' }}>{d.customer?.name ?? '—'}</span>
            {isLivr && <span style={{ fontSize: '9px', fontWeight: 700, padding: '1px 5px', borderRadius: '3px', background: '#EFF6FF', color: '#1D4ED8', border: '1px solid #BFDBFE', flexShrink: 0 }}>Livrable</span>}
          </div>
      ...
      {(d.status === 'processed' || d.status === 'rejected') && (
        <button
          disabled={saving === d.id}
          onClick={() => onStatusChange(d.id, 'pending')}
