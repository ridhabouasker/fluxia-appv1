...
import { CardHandlers, DocRow, STATUS_CFG } from './types'
      ...
      <table className="w-full border-collapse">
        <thead>
          <tr className="bg-[#F8FAFC] border-b border-[#E2E8F0]">
            {['Client','Type','Fichier','Format','Poids','Période','Statut','Date','Actions'].map(h => (
        ...
        <tbody>
          ...
          {docs.map(d => {
            ...
            const openEdit = () => setEditQual?.({
              id: d.id,
              typeId: d.type?.id ?? '',
              year:   d.year,
              month:  d.months?.[0]?.toString() ?? '',
            })
            ...
            return (
              <tr key={d.id} className={`border-b border-[#F1F5F9] last:border-0 transition-colors ${isEditing ? 'bg-[#EFF6FF]' : 'hover:bg-[#F8FAFC]'}`}>
                ...
                <td className="px-3 py-2 max-w-[150px]">
                  {isEditing && eq ? (
                    ...
                    <button onClick={openEdit} className="edit-cell-btn" style={{ background: 'none', border: 'none', padding: '2px 4px', cursor: 'pointer', textAlign: 'left', borderRadius: '4px', display: 'flex', alignItems: 'center', gap: '4px' }}>
                      ...
                      {isLivr && <span style={{ marginLeft: '2px', fontSize: '9px', fontWeight: 700, padding: '1px 5px', borderRadius: '3px', background: '#EFF6FF', color: '#1D4ED8', border: '1px solid #BFDBFE' }}>Livrable</span>}
                ...
                <td className="px-3 py-3">
                  <RowActions doc={d} saving={saving} {...handlers} />
                </td>
              </tr>
            )
          })}
